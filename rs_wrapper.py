...
class RealSenseWrapper:
    def __init__(self, height, width, fps, device_name=None):
        print('Settings up realsense pipeline...')
        self.pipeline = rs.pipeline()
        self.config = rs.config()
        if device_name:
            logger.info(f'Enable device: {device_name}')
    ...
    def wait_for_data(self) -> Optional[Tuple]:
        ...
        img_time = depth_frame.get_timestamp() / 1e3  # ms -> s
        color_image = np.asanyarray(color_frame.get_data())
        depth_image = np.asanyarray(depth_frame.get_data())  # mm -> m
